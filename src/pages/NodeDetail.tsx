...
const VerdictCTABlock = ({ nodeId }: { nodeId: string }) => {
  ...
  return (
    ...
    >
      ...
      <ul className="space-y-2 mb-6 text-grey-400 text-base">
      ...
      <p className="text-grey-300 text-base font-medium mb-6">
      ...
      <p className="text-grey-600 text-base mt-4">
      ...
      <main className="relative z-10 pt-32 pb-24 px-6">
        <div className="max-w-3xl mx-auto">
          ...
          >
            ...
            {node.domain && <p className="text-[11px] uppercase tracking-[0.3em] text-grey-300 mb-6">{node.domain}</p>}
          ...
          {isLive && node.whatYouGet && node.whoItsFor ? (
            <>
              ...
              {node.id === 'ghost-protocol' && node.description && (
                <motion.section initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="mb-12">
                  <div className="glass-card p-6 border-l-2 border-grey-600">
                    ...
                    <p className="text-grey-300 text-base mt-3">No execution. No representation. No impersonation. No deception.</p>
                  </div>
                </motion.section>
              )}
              ...
              <motion.section initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="mb-12">
                <h2 className="text-base uppercase tracking-[0.4em] text-primary mb-6">What You Get</h2>
              ...
              <motion.section initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="mb-12">
                <h2 className="text-base uppercase tracking-[0.4em] text-grey-400 mb-6">Who It's For</h2>
                <div className="glass-card p-6">
                  <ul className="space-y-4">
                    {node.whoItsFor.map((item, i) => (
                      <li key={i} className="flex items-start gap-3">
                        <span className="text-grey-300 mt-1">◆</span>
            ...
            <>
              <motion.section initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="mb-16">
                <div className="glass-card p-8 border-grey-700/50">
                  ...
                  <p className="text-grey-300 text-base">This node is currently sealed. Submit a request to express interest in access when capacity becomes available.</p>
                </div>
              </motion.section>
            ...
            <>
              <motion.section initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="mb-16">
                <div className="glass-card p-8 border-grey-800/30 opacity-70">
                  ...
                  <p className="text-grey-300 leading-relaxed mb-8">{node.description || node.purpose}</p>
                  <div className="border-t border-grey-800/30 pt-6">
                    <p className="text-grey-600 text-base mb-4">This infrastructure is intentionally reserved for future activation. Capacity has been allocated; conditions for deployment have not yet been met.</p>
                    <p className="text-grey-700 text-base">Dormant nodes represent planned expansion into regulated, capital-intensive, or irreversible decision domains.</p>
                  </div>
                </div>
              </motion.section>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
                <Link to="/request-access"><ApexButton variant="ghost" size="lg" className="w-full max-w-md mx-auto block text-grey-300">JOIN UNLOCK LIST</ApexButton></Link>
              </motion.div>
            </>
          )}
        </div>
